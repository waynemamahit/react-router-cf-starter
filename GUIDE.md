...
Add the API token as a secret in your CI/CD provider and save the resource IDs as repository variables/secrets. The pipelines in this repository use a small helper script (`scripts/inject-env.js`) to inject environment bindings into `wrangler.jsonc` before build/deploy. CI providers should store the resource values using the `STAGING_...` and `PRODUCTION_...` prefixed names described below.
...
- The repository CI jobs (GitHub Actions, GitLab CI, Bitbucket Pipelines) first build the application, which generates `build/server/wrangler.json`. Then they export the prefixed CI variables into unprefixed environment variables and call `node scripts/inject-env.js <env>` (where `<env>` is `staging` or `production`). That script reads the unprefixed env vars or `.dev.vars` (for local dev) and injects resolved values into the built `build/server/wrangler.json` before deploying.

Example (what the pipeline does under the hood):

```
# map CI variable to expected env var
...
Local development
- For local preview you can create a `.dev.vars` file in the repo root and the injector script will read values from it when invoked with `local`.
...
`pnpm preview` serves the production build using the worker/server bundle. It can be configured to read `.dev.vars` to connect to staging resources.
