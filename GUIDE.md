...
- The repository CI jobs (GitHub Actions, GitLab CI, Bitbucket Pipelines) first build the application. Then they call `node scripts/generate-wrangler.js <env>` (where `<env>` is `staging` or `production`).
...
`pnpm preview` serves the production build using the worker/server bundle. It uses `scripts/generate-wrangler.js local` to generate a temporary `wrangler.json` from your `.dev.vars` and runs `wrangler dev --local`.
