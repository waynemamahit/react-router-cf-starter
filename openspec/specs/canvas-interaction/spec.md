...
#### Scenario: List displays drawing metadata
- **WHEN** saved drawings are displayed in the list
- **THEN** each drawing shows its name and creation timestamp
- **AND** the metadata is formatted in a readable way
- **AND** drawings are ordered by creation date (most recent first)
